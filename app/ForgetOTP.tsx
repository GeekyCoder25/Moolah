...
const ForgetOTP = () => {
	...
	useEffect(() => {
		...
		const getClipboard = async () => {
			...
			console.log(clipboard, savedOTP);
	...
	const handleSubmit = async (isClipboard: boolean) => {
		try {
			...
			setLoading(true);
			const response = await axiosClient.post<{email: string; otp: string}>(
				'/auth/verify/email/',
				{email, otp}
			);
				...
				<View className="flex-row justify-between gap-x-3 mt-5 max-w-[350px]">
					<TouchableOpacity onPress={() => inputRef.current?.focus()}>
						<TextInput
							...
							value={otpCode1}
							autoFocus
							className={`border-[1px] w-24 h-24 rounded-2xl text-5xl p-1 font-bold ${
